...
- Что изменено: `PrebuiltCache.StartRefresh()` больше не прогревает все пары одновременно по одному `ticker`. Теперь refresh идёт постепенно: target/SNI-пары дедуплицируются, перемешиваются на каждом цикле и обновляются по одной с jittered delay внутри общего окна `RefreshInterval`.
- Где: `REALITY/prebuilt_cache.go`.
- Остаточный риск: регулярный burst по всему набору target'ов устранён, но долгоживущий сервер всё ещё оставляет статистически наблюдаемый background refresh traffic, особенно если набор target'ов мал и окно обновления стабильно.
...
- Что изменено: для малых payload `H2Padder` больше не использует ступенчатое округление к нескольким фиксированным bucket sizes. Вместо этого small-payload ветка сэмплирует итоговый target из плавающего ratio range и абсолютного additive jitter, иногда смешивая его с ближайшим anchor window.
- Где: `REALITY/h2_padding.go`.
- Остаточный риск: заметные ratio-кластеры для tiny payload смягчены, но общая anchor-ориентированность H2 profile остаётся и всё ещё может статистически отличаться от конкретного upstream HTTP/2 приложения на больших выборках.

### Low

#### 16. Переходный эффект первых соединений

- Что изменено: `TimingNormalizer` больше не выключает delay полностью до достижения `minSamples`. После первого живого RTT он переходит в bootstrap-режим и использует более широкий provisional target/jitter/spike profile, пока не накопит достаточно реальных сэмплов.
- Где: `REALITY/timing_normalizer.go`.
- Остаточный риск: первые соединения стали ближе к основному профилю, но bootstrap-режим всё ещё шире и менее точен, чем steady-state распределение после накопления полноценного RTT window.
