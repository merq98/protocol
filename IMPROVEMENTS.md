...
#### 2. Нереалистичная структура сертификата

- Проблема: auth path использует синтетический сертификат с пустым `SerialNumber`, пустыми DN-полями, отсутствием SAN/SCT и дополнительными артефактами вроде padding-extension для ML-DSA варианта.
- Где: `REALITY/handshake_server_tls13.go`.
- Как детектят: DPI или активный пробер парсит цепочку сертификатов и видит структуру, не похожую на публичные TLS-сертификаты реальных CDN/сайтов.
- Риск: это сильный структурный индикатор, особенно при активной проверке.
...
- Проблема: допустимое число бесполезных CCS задаётся через `GlobalMaxCSSMsgCount`, а поведение при избытке CCS отличается от типичного сервера.
- Где: `REALITY/record_detect.go`, `REALITY/tls.go`.
- Как детектят: пробер шлёт 2, 15, 16, 32 CCS и сравнивает реакцию с эталонным TLS-сервером.
- Риск: поведение по CCS можно использовать как активный fingerprint.

#### 5. Timing normalizer + prebuilt cache создают слишком ровный RTT

- Проблема: prebuilt cache уменьшает настоящий RTT почти до нуля, а normalizer затем искусственно добавляет задержку до среднего target RTT.
- Где: `REALITY/prebuilt_cache.go`, `REALITY/timing_normalizer.go`, `REALITY/tls.go`.
- Как детектят: auth path показывает подозрительно стабильный handshake latency, тогда как реальное проксирование всегда имеет естественный разброс.
- Риск: аномально низкая дисперсия RTT пригодна для ML-классификации.
